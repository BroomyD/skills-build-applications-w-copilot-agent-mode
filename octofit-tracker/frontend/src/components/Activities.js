...
function Activities() {
  ...
  const fetchData = () => {
    ...
    fetch(url).then((r) => r.json()).then((data) => {
      ...
      setItems(list);
    }).catch((err) => console.error('Activities fetch error:', err)).finally(() => setLoading(false));
  };
        ...
        <>
          <div className="modal show" style={{display:'block'}} tabIndex="-1">
            <div className="modal-dialog modal-lg"><div className="modal-content">
              <div className="modal-header"><h5 className="modal-title">Activity details</h5><button type="button" className="btn-close" aria-label="Close" onClick={closeModal}></button></div>
