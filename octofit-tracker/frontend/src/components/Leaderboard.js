  ...
  if (typeof firstObj !== 'object') {
    return (
      <div className="table-responsive">
        <table className="table table-striped table-clickable"><thead><tr><th>Value</th></tr></thead>
...
function Leaderboard() {
  ...
  const fetchData = () => {
    ...
    fetch(url).then((r) => r.json()).then((data) => {
      ...
      setItems(list);
    }).catch((err) => console.error('Leaderboard fetch error:', err)).finally(() => setLoading(false));
  };
        ...
        <>
          <div className="modal show" style={{display:'block'}} tabIndex="-1">
            <div className="modal-dialog modal-lg"><div className="modal-content">
              <div className="modal-header"><h5 className="modal-title">Leaderboard details</h5><button type="button" className="btn-close" aria-label="Close" onClick={closeModal}></button></div>
