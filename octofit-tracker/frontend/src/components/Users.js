...
function renderTable(items, onRowClick) {
  if (!items || items.length === 0) {
    return <div className="centered-empty small-muted">No data</div>;
  }
    ...
    return (
      <div className="table-responsive">
        <table className="table table-striped table-clickable">
          ...
          <tbody>
            {items.map((it, idx) => (
              <tr key={idx} onClick={() => onRowClick && onRowClick(it)}>
                <td>{String(it)}</td>
              </tr>
            ))}
          </tbody>
        ...
        <tbody>
          {items.map((row, idx) => (
            <tr key={idx} onClick={() => onRowClick && onRowClick(row)}>
              {headers.map((h) => <td key={h}>{row[h] !== undefined ? String(row[h]) : ''}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
